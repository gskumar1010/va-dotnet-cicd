...
GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-gogs}
GITHUB_REF=${GITHUB_REF:-master}
GOGS_SVC_URL=${GOGS_SVC_URL:-gogs}



function deploy() {
  oc $ARG_OC_OPS new-project dev-$PRJ_SUFFIX   --display-name="VA DOTNET Project - Dev"
  oc $ARG_OC_OPS new-project stage-$PRJ_SUFFIX --display-name="VA DOTNET Project - Stage"
...
function patch_build_config() {
  ...
  oc patch bc/va-dotnet-project-bc -p '{"spec":{"source":{"git":{"uri": "http://'"$GOGS_HOSTNAME"'/gogs/sample-db-conn.git"}}}}' -n dev-$PRJ_SUFFIX
}
